...
class AttendanceViewModel extends GetxController {
  ...
  Future<void> _handleAllBackgroundTasks(String attendanceId) async {
    ...
    try {
      ...
      // Save to local database WITH lat, lng, date, time, and battery
      // ✅ FIX: must await — otherwise postDataFromDatabaseToAPI() below can run
      // before this insert actually lands in SQLite (race condition).
      await addAttendance(
        AttendanceModel(
          attendance_in_id: attendanceId,
          user_id: user_id,
      ...
      debugPrint("✅ [ATTENDANCE] Saved (confirmed in DB): lat=$realLat, lng=$realLng, time=$clockInNow, battery=$batteryLevel%");
  ...
  // ✅ FIX: was `void` and didn't await the DB insert, so the caller in
  // _handleAllBackgroundTasks raced ahead and called postDataFromDatabaseToAPI()
  // before this row was actually committed to SQLite — that's why the
  // freshly clocked-in record was sometimes missing from the post.
  Future<void> addAttendance(AttendanceModel attendanceModel) async {
    await attendanceRepository.add(attendanceModel);
