...
// // import '../../Tracker/battery_monitor.dart';
...
// //   }
// //
// //   Future<bool> _blockIfAlreadyClockedIn(BuildContext context) async {
// //     final prefs = await SharedPreferences.getInstance();
// //
// //     // First, run any missed-clockout recovery so state is clean
// //     await _checkForMissedClockout();
// //     await _checkForMultiDayMissed();
// //
...
// //
// //     if (!isClockedIn) return false; // all clear, allow clock-in
// //
// //     // Still clocked in → block with a friendly dialog
// //     final clockInTimeStr =
// //         prefs.getString('clockInTime') ?? prefs.getString('flutter.clockInTime');
// //     final clockInTime = clockInTimeStr != null ? DateTime.tryParse(clockInTimeStr) : null;
// //
// //     final clockInLabel = clockInTime != null
// //         ? DateFormat('dd MMM, hh:mm a').format(clockInTime)
// //         : 'pichli baar';
// //
...
// //           const SizedBox(width: 12),
// //           const Expanded(
// //             child: Text('Pehle Clock-Out Karen',
...
// //               ),
// //               child: Text(
// //                 'Aap pehle se clocked-in hain ($clockInLabel).\n\n'
// //                     'Dobara clock-in karne se pehle clock-out zaroori hai.\n'
// //                     'Data save ho jayega, phir naya clock-in kar sakte hain.',
...
// //             label: const Text('Theek Hai, Clock-Out Karta Hoon'),
// //             style: ElevatedButton.styleFrom(
// //               backgroundColor: Colors.blueGrey.shade700,
// //               foregroundColor: Colors.white,
...
// //     if (await _blockIfAlreadyClockedIn(context)) return;
// //
...
// //       // ✅ Start battery monitor for 3% auto-save
// //       await BatteryMonitor.start();
// //       debugPrint("✅ [CLOCK-IN] Battery monitor started");
// //
...
// //     debugPrint('🛑 [CLOCK-OUT] Battery monitor stopped');
// //
...
// //       debugPrint('✅ [BG CLOCKOUT] Attendance saved successfully');
// //
// //       // ✅ FAKE GPS SYNC - Try to sync pending records
// //       if (reason.contains('Fake GPS')) {
// //         debugPrint('🔄 [BG CLOCKOUT] Syncing pending fake GPS records...');
// //         await FakeGpsLog.syncPending();
// //
// //         // ✅ Also save from SharedPreferences if not already in DB
...
// //           await _saveFakeGpsToLocalDbFromPrefs(prefs, clockOutTime);
// //         }
// //         debugPrint('✅ [BG CLOCKOUT] Fake GPS sync complete');
// //       }
// //
...
//
//         // ✅ Also save from SharedPreferences if not already in DB
//         // FIX: read as string (matches Kotlin's putString fix) — getDouble
//         // here always returned 0.0 because Kotlin stored these as putFloat,
//         // which Flutter's getDouble cannot decode.
//         final fakeLat = double.tryParse(prefs.getString('flutter.fake_gps_lat') ?? '') ?? 0.0;
//         final fakeLon = double.tryParse(prefs.getString('flutter.fake_gps_lon') ?? '') ?? 0.0;
//         if (fakeLat != 0.0 || fakeLon != 0.0) {
//           await _saveFakeGpsToLocalDbFromPrefs(prefs, clockOutTime);
//           // FIX: the syncPending() above ran BEFORE this row existed, so it
//           // never got posted. Sync again now that the row is in the DB.
//           await FakeGpsLog.syncPending();
//         }
//         debugPrint('✅ [BG CLOCKOUT] Fake GPS sync complete');
//       }
...
//   Future<void> _saveFakeGpsToLocalDbFromPrefs(SharedPreferences prefs, DateTime eventTime) async {
//     try {
//       // FIX: Kotlin now writes these as putString(...), not putFloat(...).
//       // getDouble() on a string-backed key always returns null, which is
//       // why this previously silently no-op'd. Parse the string instead.
//       final fakeLat = double.tryParse(prefs.getString('flutter.fake_gps_lat') ?? '') ?? 0.0;
//       final fakeLon = double.tryParse(prefs.getString('flutter.fake_gps_lon') ?? '') ?? 0.0;
//       final realLat = double.tryParse(prefs.getString('flutter.real_gps_lat') ?? '') ?? 0.0;
//       final realLon = double.tryParse(prefs.getString('flutter.real_gps_lon') ?? '') ?? 0.0;
//
//       if (fakeLat == 0.0 && fakeLon == 0.0) return;
//
//       final userId = prefs.getString('user_id') ?? prefs.getString('userId') ?? '';
...
class _TimerCardState extends State<TimerCard> with WidgetsBindingObserver {
  ...
  Future<void> _checkForBackgroundClockout() async {
    ...
    // FIX: Kotlin writes fastClockOutDistance via putFloat(); Flutter's
    // getDouble() cannot decode a native putFloat() value — always returns 0.0.
    // prefs.get() returns the raw stored value (a double on Dart side, a float
    // boxed as num when written natively), casting via (num?) handles both cases.
    final distance = (prefs.get('flutter.fastClockOutDistance') as num?)?.toDouble() ?? 0.0;

    debugPrint('⚡ [BG CLOCKOUT] Detected: $reason | time=$clockOutTime | dist=$distance');
    ...
    // FIX: Temporarily restore isClockedIn = true so fastSaveAttendanceOut
    // does not early-return if it guards on this flag. After app kill, the
    // ViewModel starts with isClockedIn = false (fresh GetX instance) even
    // though the user was still clocked in — without this the save no-ops.
    locationViewModel.isClockedIn.value   = true;
    attendanceViewModel.isClockedIn.value = true;
