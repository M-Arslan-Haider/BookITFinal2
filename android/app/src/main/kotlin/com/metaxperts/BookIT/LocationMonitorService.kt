...
////    // Every 5 minutes, write a local row {user, city, designation, battery%,
////    // status="online", timestamp}. Pure local DB write — works fully offline.
////    // The bulk poster (PowerStatusBulkPoster) syncs these rows when network is
////    // available. Gaps between consecutive timestamps in the table represent
////    // periods the device/app was OFF (no heartbeat could be written).
////    private val POWER_STATUS_INTERVAL_MS  = 5 * 60_000L   // 5 minutes
////    private val POWER_STATUS_FIRST_DELAY  = 10_000L       // first beat 10s after start
////
////    // ─── SharedPreferences Keys ───────────────────────────────────────────────
////    private val PREFS_NAME              = "FlutterSharedPreferences"
////    private val KEY_IS_CLOCKED_IN       = "flutter.isClockedIn"
...
////
////    private val nativeDb: NativeDBHelper by lazy { NativeDBHelper(applicationContext) }
////    private val powerStatusDb: PowerStatusDBHelper by lazy { PowerStatusDBHelper(applicationContext) }
...
////            MidnightClockoutReceiver.schedule(this)
////            LocationUploadWorker.schedule(this)
////            PowerStatusBulkPoster.schedule(this)
////            // OEM-kill resilience: self-repeating exact-alarm watchdog that
////            // restarts this service even if the process is silently killed.
...
////        mqttPublishRunnable?.let  { handler.removeCallbacks(it) }
////        mqttReconnectRunnable?.let{ handler.removeCallbacks(it) }
////        powerStatusRunnable?.let  { handler.removeCallbacks(it) }
////        handler.removeCallbacksAndMessages(null)
////        stopLocationUpdates()
////        try { (getSystemService(NotificationManager::class.java)).cancel(WORKING_NOTIFICATION_ID) } catch (_: Exception) {}
////    }
...
////
////    // ═════════════════════════════════════════════════════════════════════════
////    // Power-Status Heartbeat — every 5 minutes, "online" + battery + timestamp
////    //
////    // Pure local SQLite write (no network needed) → works fully offline.
////    // While the foreground service is alive (device powered ON, app running,
////    // user clocked in) a row is appended every 5 minutes with status="online".
////    // The moment the device powers off / the service is killed, no more rows
////    // are written. On reboot, BootCompletedReceiver restarts this service,
////    // which resumes the heartbeat — the GAP between the last "online" row
////    // before shutdown and the first "online" row after restart IS the
////    // power-off period (no DB writes possible while the device was off).
////    // ═════════════════════════════════════════════════════════════════════════
////
////    private fun startPowerStatusHeartbeat() {
////        powerStatusRunnable?.let { handler.removeCallbacks(it) }
////        powerStatusRunnable = object : Runnable {
////            override fun run() {
////                if (isDestroyed) return
////                val prefs = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
////                if (prefs.getBoolean(KEY_IS_CLOCKED_IN, false) &&
////                    !prefs.getBoolean(KEY_IS_TIMER_FROZEN, false)) {
////                    ioExecutor.submit { savePowerStatusToDB(prefs) }
////                }
////                if (!isDestroyed) handler.postDelayed(this, POWER_STATUS_INTERVAL_MS)
////            }
////        }
////        handler.postDelayed(powerStatusRunnable!!, POWER_STATUS_FIRST_DELAY)
////        log("✅ [PowerStatus] Heartbeat scheduled — every ${POWER_STATUS_INTERVAL_MS / 60_000L} min")
////    }
////
...
////        } catch (e: Exception) {
////            log("⚠️ [PowerStatus] Battery read failed: ${e.message}")
////            -1.0
////        }
////    }
////
////    private fun savePowerStatusToDB(prefs: android.content.SharedPreferences) {
////        val wl = acquireShortWakeLock("BookIT::PowerStatusWriteLock", 10_000L)
////        try {
////            // NOTE: "city" isn't currently stored by Flutter's identity write-back.
////            // Add `.putString("flutter.city", city)` on the Flutter side (same place
////            // userName/userDesignation are saved) to populate this column.
////            val city      = prefs.getString("flutter.city", "") ?: ""
////            val battery   = getBatteryLevel()
////            // Backend expects: TO_TIMESTAMP(v_log_ts, 'YYYY-MM-DD"T"HH24:MI:SS')
////            val timestamp = SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss", Locale.getDefault()).format(Date())
////
////            powerStatusDb.insertPowerStatus(
////                userId      = userId,
////                userName    = bookerName,
////                city        = city,
////                designation = designation,
...
////                timestamp   = timestamp
////            )
////            log("🔋 [PowerStatus] Saved heartbeat — battery=$battery% status=online @ $timestamp")
////        } catch (e: Exception) {
////            log("❌ [PowerStatus] Save failed: ${e.message}")
////        } finally {
////            releaseWakeLock(wl)
////        }
////    }
...
//            log("✅ [GPS] Native GPS_PROVIDER started — interval=${gpsPolicy.locationIntervalSec}s accuracy=$maxAccuracy m")
//
//            // FIX (instant mock-location detection): PASSIVE_PROVIDER receives
//            // every location fix that ANY app/provider on the device produces —
//            // including fixes injected by a mock-location app — the moment they
//            // happen, at zero extra cost (no new GPS hardware request). Without
//            // this, mock location was only caught on the next scheduled
//            // GPS_PROVIDER tick (up to gpsPolicy.locationIntervalSec, i.e. up to
//            // 60s later), both in foreground and background.
//            try {
//                lm.requestLocationUpdates(
...
//                    Looper.getMainLooper()
//                )
//                log("✅ [GPS] PASSIVE_PROVIDER listener attached for instant mock detection")
//            } catch (e: Exception) {
//                log("⚠️ [GPS] PASSIVE_PROVIDER registration failed: ${e.message}")
//            }
//        } catch (e: Exception) {
//            log("❌ [GPS] startLocationUpdates failed: ${e.message}")
//        }
...
//                .putBoolean("isClockedIn", false)
//                .putBoolean(KEY_FAKE_GPS_DETECTED, true)
//                // FIX: changed from putFloat(...) to putString(...). Flutter's
//                // SharedPreferences plugin stores/reads doubles in a format that
//                // is NOT compatible with a native Android putFloat() write —
//                // prefs.getDouble() on the Dart side returned null/0.0 for these
//                // keys 100% of the time, which silently dropped every fake-GPS
//                // event whenever the app was killed (the only place these
//                // values were ever read back from).
//                .putString("flutter.fake_gps_lat", fakeLat.toString())
//                .putString("flutter.fake_gps_lon", fakeLng.toString())
//                .putString("flutter.real_gps_lat", lastRealLat.toString())
//                .putString("flutter.real_gps_lon", lastRealLon.toString())
//                .putString("flutter.fastClockOutTime", ts)
//                .putFloat("flutter.fastClockOutDistance", 0f)
//                .putString("flutter.fastClockOutReason", "System ClockOut - Fake GPS Detected")
//                .putBoolean("flutter.hasFastClockOutData", true)
...
//                    """{"fast_attendanceId":"","fast_userId":"$userId","fast_clockOutTime":"$ts","fast_totalTime":"$elapsed","fast_totalDistance":0.0,"fast_latOut":${lastRealLat},"fast_lngOut":${lastRealLon},"fast_address":"","fast_reason":"System ClockOut - Fake GPS Detected","fast_savedAt":"${System.currentTimeMillis()}","fast_clockInTime":"$clockInT"}""")
//                .commit()
//            log("💾 [Critical] Saved: reason=System ClockOut - Fake GPS Detected ts=$ts")
//
//            // FIX: previously NOTHING posted this event to the backend unless
//            // the Flutter app was reopened (only Flutter's
//            // _saveFakeGpsToLocalDbFromPrefs/_postUnsynced did that, and only
//            // on resume). This native POST fires immediately, independent of
//            // whether the app/isolate is ever reopened, so detection while the
//            // app is killed actually reaches the server right away.
//            ioExecutor.submit {
//                postFakeGpsNative(
//                    fakeLat  = fakeLat,
//                    fakeLng  = fakeLng,
...
//    }
//
//    // ── FIX: native POST so a fake-GPS event reaches the backend even if the
//    // Flutter app is never reopened. Mirrors fake_gps_logs.dart's _post():
//    // same URL, same JSON field names, same self-signed-cert bypass scoped
//    // to this one private host only.
//    private val FAKE_GPS_API_URL =
//        "https://cloud.metaxperts.net:8443/erp/valor_trading/fakegpspost/post/"
//
...
//        try {
//            val prefs = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
//            val uid   = prefs.getString("flutter.userId", userId) ?: userId
//            val bname = prefs.getString("flutter.userName", bookerName) ?: bookerName
//            val desig = prefs.getString("flutter.userDesignation", designation) ?: designation
//
//            val body = JSONObject().apply {
//                put("user_id", uid)
//                put("booker_name", bname)
//                put("designation", desig)
//                put("real_latitude", realLat)
//                put("real_longitude", realLng)
//                put("real_address", "")
//                put("fake_latitude", fakeLat)
//                put("fake_longitude", fakeLng)
//                put("fake_address", "")
//                put("distance_km", 0.0)
//                put("detected_at", detectedAtIso)
//            }.toString()
//
//            val url = URL(FAKE_GPS_API_URL)
//            conn = (url.openConnection() as HttpsURLConnection).apply {
//                // Same scope-limited self-signed cert bypass as the Dart side —
//                // only for this specific private host, not all hosts.
//                if (url.host == "cloud.metaxperts.net") {
//                    sslSocketFactory = trustingSslSocketFactory()
//                    hostnameVerifier = HostnameVerifier { hostname, _ -> hostname == "cloud.metaxperts.net" }
//                }
//                requestMethod = "POST"
...
//                log("✅ [FakeGPS-Native] Posted successfully → $code")
//            } else {
//                val err = try { conn.errorStream?.bufferedReader()?.readText() } catch (_: Exception) { null }
//                log("⚠️ [FakeGPS-Native] Server rejected → $code: $err")
//            }
//        } catch (e: Exception) {
//            log("❌ [FakeGPS-Native] POST failed: ${e.message}")
//        } finally {
...
class LocationMonitorService : Service() {
    ...
    // Runs on the ioExecutor (background thread) — never call on main thread.
    private fun reverseGeocode(lat: Double, lng: Double): String {
        return try {
            if (!android.location.Geocoder.isPresent()) return "$lat, $lng"
            val geocoder = android.location.Geocoder(this, java.util.Locale.getDefault())
            @Suppress("DEPRECATION")
            val addresses = geocoder.getFromLocation(lat, lng, 1)
            if (addresses.isNullOrEmpty()) return "$lat, $lng"
            val a = addresses[0]
            listOfNotNull(
                a.thoroughfare,
                a.subLocality,
                ...
                a.adminArea,
                a.countryName
            ).filter { it.isNotBlank() }.joinToString(", ").ifEmpty { "$lat, $lng" }
        } catch (e: Exception) {
            log("⚠️ [Geocoder] reverseGeocode failed: ${e.message}")
            "$lat, $lng"
        }
    }
    ...
    private fun postFakeGpsNative(
        ...
        try {
            ...
            // Resolve addresses on ioExecutor (already background) before building body
            val realAddress = reverseGeocode(realLat, realLng)
            val fakeAddress = reverseGeocode(fakeLat, fakeLng)
            log("📍 [FakeGPS-Native] realAddress='$realAddress' fakeAddress='$fakeAddress'")
            ...
            val body = JSONObject().apply {
                ...
                put("real_latitude", realLat)
                put("real_longitude", realLng)
                put("real_address", realAddress)
                put("fake_latitude", fakeLat)
                put("fake_longitude", fakeLng)
                put("fake_address", fakeAddress)
                put("distance_km", 0.0)
                put("detected_at", detectedAtIso)
            }.toString()
